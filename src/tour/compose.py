...
from .certification_provider import CallPurpose
...
from .validation import validate_source_traceability
from .verify import (
    FaithfulnessChecker,
...
class MockComposeClient:
    ...
    def compose(
        ...
        for i, sentence in enumerate(sentences):
            ...
            if (
                nxt is not None
                and nxt.stop_idx == slot
                and _is_transit_sentence(nxt, request.beats_by_id)
            ):
...
class AnthropicComposeClient:
    ...
    def __init__(
        ...
        *,
        ...
        if absolute_deadline_s is None:
            from src.tour.anthropic_client import COMPOSE_ABSOLUTE_DEADLINE_S

            absolute_deadline_s = COMPOSE_ABSOLUTE_DEADLINE_S
        if absolute_deadline_s <= 0:
            raise ValueError("compose absolute deadline must be positive")
    ...
    def compose(
        ...
        with self._client.messages.stream(  # type: ignore[attr-defined]
            **stream_options
            ...
            timer = threading.Timer(self.absolute_deadline_s, expire_stream)
            timer.daemon = True
            timer.start()
            try:
                ...
                timer.cancel()
            if expired.is_set() or time.monotonic() - started > self.absolute_deadline_s:
                raise TimeoutError("compose absolute deadline exceeded")
...
class CertificationAnthropicComposeClient:
    """One zero-retry, ledger-owned compose attempt per immutable unit id.

    This client deliberately does not implement the legacy attempt/repair loop.
    A certification coordinator calls ``compose_unit`` once for each of at most
    eight units. Any malformed, failed, incomplete, or rejected unit fails the
    candidate; this client has no repair or fallback path.
    """
    ...
    def __init__(
        ...
        *,
        ...
        model: str = COMPOSE_MODEL,
        client: object | None = None,
        absolute_deadline_s: float = 180.0,
    ) -> None:
        if absolute_deadline_s <= 0:
            raise ValueError("certification compose deadline must be positive")
        self.ledger = ledger
        self.model = model
        self.absolute_deadline_s = absolute_deadline_s
        if client is None:
    ...
    def compose_unit(
        ...
        stop_indexes = {sentence.stop_idx for sentence in request.stitched.script}
        if len(stop_indexes) != 1:
            raise ValueError("a certification compose unit must contain exactly one stop")
        stop_index = next(iter(stop_indexes))
        if stop_index is None or stop_index < 0:
            raise ValueError("a certification compose unit requires a canonical stop index")
        ...
        input_ceiling = len(envelope.encode("utf-8"))
        ...
        try:
            with self._client.messages.stream(  # type: ignore[attr-defined]
                **sdk_request
                ...
                try:
                    ...
                    timer.cancel()
                if expired.is_set() or time.monotonic() - started > self.absolute_deadline_s:
                    raise TimeoutError("certification compose absolute deadline exceeded")
            if getattr(response, "stop_reason", None) == "max_tokens":
                raise ValueError("certification compose output hit its fixed token ceiling")
            usage = getattr(response, "usage", None)
            if usage is None:
                raise ValueError("certification compose response omitted usage")
            text = next(
                (
                    block.text
            ...
            if text is None:
                raise ValueError("certification compose response carried no text block")
            try:
                data = json.loads(text)
                parsed_sentences = _sentences_from_json(data["sentences"], request)
            except Exception:
            ...
            self.ledger.complete(
                sequence,
                response_sha256=hashlib.sha256(text.encode("utf-8")).hexdigest(),
                parsed_payload_sha256=sentences_payload_sha256(parsed_sentences),
                input_tokens=int(getattr(usage, "input_tokens", 0) or 0),
                output_tokens=int(getattr(usage, "output_tokens", 0) or 0),
                latency_ms=round((time.monotonic() - started) * 1000),
                provider_request_id=getattr(response, "id", None),
            ...
            record = self.ledger.snapshot().records[sequence - 1]
...
def _certification_compose_requests(
    ...
    if not 1 <= len(stops) <= 8:
        raise ValueError("certification composition supports one to eight stops")
    all_slots = tuple(
        slot
...
def finalize_certification_composition(
    ...
    *,
    ...
    def validate_authorized_sources(
        ...
        return validate_source_traceability(
            script,
            sequence,
    ...
    def finish(final_script: Script) -> CertificationComposition:
        ...
        for stop_index in stops:
            ...
            provider_source = composed_by_stop[stop_index]
            ...
            source_indexes = tuple(range(len(source_sentences)))
            traces.append(
                CompositionTrace(
...
def compose_certification_candidate(
    stitched: Script,
    beat_sequence: BeatSequence,
    route: Route,
    *,
    client: CertificationAnthropicComposeClient,
    authoring_candidate: AuthoringCandidateIdentity,
    chunk_text_by_slug: dict[str, str] | None = None,
    max_workers: int = 6,
) -> CertificationComposition:
    """Call each authorized stop once, then delegate to durable pure replay.

    Every request is checked against the frozen full call plan before call one. Each
    stop receives one physical compose attempt; malformed, provider-failed, or
    VERIFY-failing calls propagate and fail the whole candidate. No model repair,
    grounded restoration, or surgical splice is legal here.
    """
    _beats_by_id, stops, requests = _certification_compose_requests(stitched, beat_sequence, route)
    authoring_plan = AuthoringCandidatePlan(
    ...
    authoring_by_stop = {
        item.stop_index: item for item in authoring_plan.stop_requests
    }

    planned_compose = {
        unit.unit_id: unit
        for unit in client.ledger.authorization.call_plan
        if unit.purpose is CallPurpose.COMPOSE
    }
    if set(planned_compose) != {f"stop:{index}" for index in stops}:
        raise ValueError("frozen compose call plan differs from candidate stops")
    for stop_index, request in requests.items():
        envelope, _ = candidate_compose_request_envelope(
            request,
            authoring_by_stop[stop_index],
            model=client.model,
        )
        unit = planned_compose[f"stop:{stop_index}"]
        request_hash = hashlib.sha256(envelope.encode("utf-8")).hexdigest()
        if unit.request_sha256 != request_hash:
            raise ValueError("frozen compose request hash differs from candidate")
        if unit.compose_source_sha256 != sentences_payload_sha256(request.stitched.script):
            raise ValueError("frozen compose source differs from the grounded stitch")
        if (
            len(envelope.encode("utf-8")) > unit.input_token_ceiling
            or unit.output_token_ceiling < CERTIFICATION_COMPOSE_MAX_OUTPUT_TOKENS
        ):
            raise ValueError("frozen compose token ceiling is too small for candidate")

    def compose_stop(stop_index: int) -> tuple[int, tuple[Sentence, ...]]:
        return stop_index, client.compose_unit(
            requests[stop_index],
            authoring_by_stop[stop_index],
        )

    with ThreadPoolExecutor(max_workers=max(1, min(max_workers, len(stops)))) as pool:
        composed_pairs = list(pool.map(compose_stop, stops))
    composed_by_stop = dict(composed_pairs)
    records = {
        record.unit_id: record
        for record in client.ledger.snapshot().records
        if record.purpose is CallPurpose.COMPOSE
    }
    ...
    for stop_index in stops:
        ...
        if (
            ...
            or not record.parsed_payload_sha256
        ):
            raise ValueError("final candidate lacks its completed compose response")
        completed.append(
            CompletedCertificationComposeUnit(
                ...
                authoring_request=authoring_by_stop[stop_index],
                parsed_provider_sentences=composed_by_stop[stop_index],
...
class OpenAIComposeClient:
    ...
    def compose(
        ...
        if getattr(choice, "finish_reason", None) == "length":
            raise ValueError(
                "compose output truncated (finish_reason=length) — the sentence list is incomplete"
            )
        usage = getattr(response, "usage", None)
        if usage is not None:
...
def compose_script(
    ...
    *,
    ...
    repair_fn = (lambda comp, rep: repair_composed(comp, stitched, rep)) if repair else None
...
__all__ = [
    ...
    "CertificationAnthropicComposeClient",
    "CertificationComposition",
    "CompletedCertificationComposeUnit",
    "ComposeClient",
    "ComposeRequest",
