...
**Cleaned-source dispatch (added 2026-04-25):** When the orchestrator (Phase B / wave launcher) sends a chunk path to a background agent, it must check for `<chunk>.cleaned.txt` and pass that path if it exists; fall back to the original `<chunk>.txt` otherwise. Do NOT modify the agent prompt template to do this check — keep that responsibility in the main conversation so cleanup logic stays centralized. The agent's `source_chunk_slug` field still uses the canonical chunk identity (e.g. `chunk-06-marais`), not the cleaned suffix. See `scripts/strip_pdf_artifacts.py` for the cleaning step that produces the `.cleaned.txt` files; clean inputs are durable and produced once per chunk.

Launch one Agent per chunk. **ALL agents in a single message.** Use:
- `subagent_type: "general-purpose"`
- **Hybrid model strategy (revised 2026-04-25):** Opus for the **anchor wave** (tier-5 POI chunks — Notre-Dame/Sainte-Chapelle/Conciergerie/Place des Vosges/Marais/Quartier Latin/Historic Paris); **Sonnet + Phase B fixup pass** for **filler waves** (everything else). Sonnet smoke-tested at 56% tier-3+ cue coverage, 76% Pydantic acceptance, 48% length-class self-consistency, and zero trigger_address beats on v2 — acceptable only for non-anchor content where the Phase B fixup recovers schema/length/subject_tag failures and queues genuine misses for review (do NOT auto-fill empty physical_cues — that's invention). Opus matches hand-extracted Vallois quality (chunks 01–05) on the anchor cases. Revisit Sonnet for anchor work only if a future Sonnet release passes a fresh smoke test of Rough Guide chunk-01 (≥95% Pydantic acceptance, ≥95% tier-3+ cue coverage, ≥90% length-class consistency, ≥1 trigger_address beat).
