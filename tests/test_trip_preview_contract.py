...
def test_preview_single_stop_that_cannot_fill_timebox_is_structured_422(make_client):
    ...
    The wire (trips.py::_tourability_payload) must carry the refusal — dropping it is
    what made thin-area single-stop tours look like silent bugs.
    """
    client = make_client(_lone_anchor_records())
    ...
    # The refusal names WHICH kind of "no" it is, so a surface can say "this cannot be
    # walked in the time you asked for" rather than "there is nothing to see here".
    assert detail["cause"] == "time_budget"
    assert "required 3240-3960s" in detail["reason"]
    # The refusal offers a way out. Each suggestion is complete enough to act on.
...
def test_preview_green_pool_but_materially_thin_delivery_is_422(make_client):
    ...
    detail = r.json()["detail"]
    assert detail["cause"] == "time_budget"
    assert "best eligible bounded route 2403s" in detail["reason"]
